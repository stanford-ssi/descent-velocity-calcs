function [Cd, S] = CdRocket(h, U, Ma)
	...
	v2x = U*cos(theta);
	v2y = U*sin(theta);
	FoverA(1) = (v1x^2)*rho(h) - (v2x^2)*rho(h);	% reynolds transport formula without the area term (later accounted for)
	FoverA(2) = (v1y^2)*rho(h) - (v2y^2)*rho(h);
